...
def test_linkage_plan_crud():
    ...
    # 创建预案
    plan_data = {
        "plan_name": f"测试预案_{datetime.now().strftime('%H%M%S')}",
        "org_id": TEST_ORG_ID,
...
def test_update_plan(plan):
    """测试更新预案"""
    print(f"\n=== Test 3: 更新预案 (id={plan['id']}) ===")
    
    update_data = {
        "plan_name": f"更新后的预案_{datetime.now().strftime('%H%M%S')}",
        "fire_type": "pre_fire",
...
def test_query_logs(plan):
    ...
    if total >= 2:  # 至少有 2 个动作日志（start_exhaust + close_door）
