...
- **决策**: 判定一条「生产代码报错」的严重程度时，**必须先查清调用方**（谁、在哪条路径下会走到这里）。
  确认不可达后，按「**潜伏缺陷**」记录并建议降级，而不是按报错点所在的分层（service / api）想当然定级；
  **记录本身保留**，不因降级而删除。
- **原因**:
  1. P1-010 被记为「**线上 500**」并挂了 6 天。实测：端点 `POST /notifications/read-all` **自带内联实现**
     （`notifications.py:80` 有自己的 `from sqlalchemy import update`），而真正报错的
     `emergency_service.mark_notifications_as_read`（`:449` 定义 / `:467` 报错）**没有任何端点调用**——
     全仓库仅 3 类命中：函数定义、`notifications.py:19` 的**未使用 import**、`tests/test_notifications.py` 的 5 处调用。
  2. **错因是可复用的**：从失败用例倒推时，只看到 `notifications.py:19` 有 import 就断定端点在调它，
     **没读端点函数体**。与「看到函数名就以为被调用」同源。
  3. 本会话用**三层证据**独立坐实：**静态**（全仓库检索调用方）+ **结构**（读路由函数体，确认内联实现）+
     **动态**（容器实测 `POST /api/v1/notifications/read-all` → **HTTP 200**）。
     `docker-compose.yml` 把 `./backend` 挂载为 `/app`，容器内代码即工作树，故容器实测对本仓库有效。
- **影响范围**: `CLAUDE.md` P1-010 备注；`docs/test/2026-09-19-前后端测试结果汇总.md` 第八节
- **来源会话**: 2026-09-19 13:53
- **⚠️ 连带**:
  1. 同类判据应推广到 **P2-021 的 A 类 3 条**（同样是「测试在测一个没人用的函数」），以及所有「测试红但可能不可达」的记录。
  2. **反向风险**：把生产缺陷误记成测试缺陷，会让它从「待修」变成「不用修」——
     P2-021 原记的「全在测试侧」正是此形态，比数字记错更危险。
