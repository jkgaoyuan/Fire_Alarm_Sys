...
| P1-010 | `emergency_service.py` 少 import `update` 导致 NameError | P1 | [ ] | 2026-09-13 | **2026-09-19 影响面已确认（本条悬了 6 天的那个问题有答案了）**：就是**通知已读链路**。`emergency_service.py` 第 14 行只有 `from sqlalchemy import select, func`，而第 **466** 行（原记 392，文件已增长）直接 `stmt = update(NotificationModel).where(...)` → **`POST /api/v1/notifications/read-all`（`notifications.py:73-74`）必然 500**。前端 `frontend/src/api/emergency.js` 的 `markNotificationsAsRead()` 正调它。2026-09-19 实测：`pytest tests/test_notifications.py` 的 4 条红里有 2 条报此错。**是 1 行 import 的修法**，本轮未做。 实测报错 `NameError: name 'update' is not defined`，位置 `app/services/emergency_service.py:392`。**该文件顶部只有 `from sqlalchemy import select, func`**，函数内只局部 import 了模型。⚠️ **影响面未确认**——需先确认哪个端点会走到此处（若是通知已读链路，则是线上 500）。2026-09-13 由测试套件失败暴露，非新引入。修法：补 `update` 到顶部 import 并加回归用例 |
