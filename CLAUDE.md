...
| P2-021 | 剩余 9 条既存红（测试侧三类根因） | P2 | [ ] | 2026-09-19 | 当前全量 9 failed 的构成见 **P1-012** 备注。**全在测试侧**：① `test_emergency_fixtures.py` / `test_emergency_event.py` —— `EmergencyTimeline(user_id=...)`（真实字段 `operator_id`，P1-011 记过的同类漂移，这次漂移的是**测试**）、`SQLite DateTime type only accepts Python datetime`、一条断言写错（`assert '1' == 'field_confirmed'`）；② `test_notifications.py` —— 2 条撞 P1-010 的 NameError + 2 条测试文件缺 import（`mark_notifications_as_read` 未导入）。这批此前同样被 P1-013 挡着跑不到 |
