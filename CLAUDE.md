---
  ...
  4. **Celery 技术栈说明**：参考项目根目录 `消防监控管理系统_PRD_v2.1.md` 的 `2.2 Celery 引入时机说明`
     - ⚠️ 该文件已由 v2.0 原地修订为 **v2.1（实现对齐版，2026-09-19）**：FRD 逐条加了「实现状态」列，
       新增 **§3.11 实现偏离明细**（含 4 类缺口形态、逐条偏离、技术选型偏离、PRD 自身错误更正）。
       **读 PRD 前先看 §0 修订说明与 §3.11**——v2.0 有多处描述与代码不符（MinIO、一批 API 路径、
       角色权限矩阵、ER 图等），§3.11.4 列了全部更正项
...
- **2026-09-19 13:53 归档（前后端全量测试汇总 + 9 条失败根因订正 + P1-010 影响面独立复核）**：用户要求「汇总前后端测试结果存到 `docs/test`」，产出 `docs/test/2026-09-19-前后端测试结果汇总.md` + 两份原样落盘的原始产物（`raw/pytest-2026-09-19.txt`、`raw/vitest-2026-09-19.json`）。**本会话未改任何被测源码，测试数字与上一份归档（12:59）逐条一致、零增量**：后端 **470 passed / 9 failed / 60 skipped / 3 xfailed**（542 条，65 文件 = 60 单测 + 5 e2e；两次独立运行 166.72s / 156.11s 的计数与 `FAILED` 列表**逐条相同**，**又一次未复现 P1-012 记的「套件不稳定」**）、前端 **53 files / 397 passed / 0 failed**、`npm run build` 退出码 0。**9 条失败 = 6 个根因**：**① 指向生产代码 1 个根因 / 3 条**（`emergency_service.py:467` 缺 `update` import）；**② 测试自身 5 个根因 / 6 条**（`EmergencyTimeline(user_id=)` 应为 `operator_id`、**实参顺序传反**、SQLite 拒绝字符串日期 ×2、测试直接触碰惰性关系 `event.timelines` 触发 `MissingGreenlet`、测试文件缺 import）。据此**订正 CLAUDE.md 三条待办**：**P1-010**——「2 条测试红」→ **3 条**（第 4 条是测试文件自己漏 import，**属另一个根因**），补两条证据（全仓库检索确认**无任何端点调用**该函数；**容器实测** `read-all` 返回 **HTTP 200**），并**删掉同一单元格内自相矛盾的旧尾巴**（前半段已改「端点正常」，后半段仍留着「⚠️ 影响面未确认……则是线上 500」与过时行号 `:392`）；**P1-012**——应急根因「三类」→ **四类**（漏了 `MissingGreenlet`），通知构成 `2+2` → **`3+1`**；**P2-021**——「全在测试侧 / 三类根因」→ **「6 个根因，其中 1 个根因 / 3 条指向生产代码」**，「一条断言写错」→ **实参顺序传反**（`add_timeline_node` 签名 `(session, event_id, node_type, operator_id)`，测试传了 `(1, "field_confirmed")`，**断言本身是对的**）。**新增 DEC-031**（缺陷严重程度**先确认可达性再定级**）。⚠️ **本会话开工时读到的 P1-010 还是旧结论，中途该条已被 peer 会话先行更正**（更正方向与本会话独立复核一致）——本会话的增量是**独立确认 + 补 2 条新证据 + 修掉更正后仍不准的计数**。⚠️ **三条方法论教训**：**①** 订正长备注要**读完整个单元格**——P1-010 的更正只改了开头，互相矛盾的旧尾巴留在原处；**②** 把生产缺陷误记成测试缺陷会让它从「待修」变成「不用修」（P2-021 原记「全在测试侧」正是此形态，**比数字记错更危险**）；**③** 本会话自身**两次先写结论、后核对源码**（把「实参顺序传反」误写成「断言写错」；把 3 条 xfail 全当成 `strict=True`，实际 `test_inspection_api.py` 那条是 **`strict=False`**），均在核对后订正并记入会话文件。**池中移除 P1-013**（`bigint_pk()` 已落地、全量套件内 `NOT NULL constraint failed: *.id` 已全消失，本条已过 1 个归档周期）。⚠️ **待用户决策**：P1-010 优先级列仍为 `P1`，备注已写明「建议 P1 → P2」（函数不可达、1 行 import 即可修、与 P2-021 的 A 类 3 条重叠），但**改列属排期判断而非事实订正**，未擅自改。会话文件见 `.claude/sessions/2026-09-19-1353.md`。

- **2026-09-19 12:59 归档（大屏「报警设备」为 0 的根因 + 应急执行入口 + 值班员菜单码 + 报告导出 500）**：一个用户提问滚成 **6 个提交**。**① 查清「模拟火警不上大屏统计」**：大屏「报警设备」卡片读的是 `devices.status`（`monitor_service.py:104-114` 按 **devices 表**聚合），而该字段全仓库**唯一写入方是上报入库** `device_report_service.py:97`；模拟端点走 `raise_alarm`（`alarm_service.py:134-171`）**只 INSERT `alarms` 行、从不碰 `device.status`** → 两层数据源不同，卡片必然为 0。另有**三处 `is_drill` 过滤**（`AlarmList.vue:75`、`stores/monitor.js:76`、`monitor_service.py:122`）全部源于 **FR-045（P0 演练数据隔离）**，是**设计而非缺陷**。**并用真实火警实测对照坐实**：`POST /monitor/report` 造 `alarm_id=6` → 设备 `normal→alarm` → 卡片 **0→1**。**② 模拟端点补 `alarm_new` 广播**（`cd8ea68a`）：`linkage_plans.py` 全文 `publish`/`redis` **零命中**，而 `af3a8e88` 的提交信息把「**不广播**」列在**要修的毛病**那侧；`alarm_payload` 带 `is_drill`（`alarm_service.py:99`）、`Center.vue:363/402` 把 WS 帧与 REST 结果合并后统一按开关过滤——说明**频道本就该传演练帧**，是漏了。补广播前先堵两个大屏洞（`pendingFireCount` 徽标、`activeAlarmDeviceIds` 地图着色），否则演练会让「**徽标 1、列表空**」。**③ 应急事件页补「执行」入口**（`ec901531`）：后端 8 个端点里 `resolve`/`close`/`timelines`(写) **前端零入口**——`api/emergency.js` 没有 `resolveEvent`/`closeEvent`，`TimelineEditor` 全仓库只被 `Event.vue:114` 引用且写死 `:read-only="true"`；两个封装 body 处理**刻意相反**（`resolve` 后端 `payload: dict` 必填，缺 body 直接 422；`close` 后端不收 body）。**④ 值班员两处菜单码漏授**（`e4c084b9`/`765df519`）：`ROLE_PERM_MAP["duty_officer"]` 给了 `emergency:view`/`timeline` 却**没给菜单码 `emergency:event`**，联动组同样缺 `linkage:plan`——菜单树只取 `perm_type='menu'` 且**不做「父菜单随子权限自动补齐」**（`permission_service.build_menu_tree`），故按钮权限**等于白给**；已应用到开发库并以 `duty01` 实测菜单树出现两个新入口。**⑤ 守卫抓出幽灵码 `statistics:partial`**：只在两个角色清单里、**没有任何定义**，绑定的 `if c in perm_map` **静默跳过**（真实库 `count(*)=0`，全仓库零引用）→ **删除而非补定义**（补了只会新增无人校验的孤儿码）。**⑥ 新增守卫 `tests/test_role_permission_hierarchy.py`**：「凡角色持有的权限码，其 `parent_code` 也必须持有」——能写成**严格规则、零豁免名单**（修完 ④⑤ 后三角色 77 码零违规），覆盖 **P2-012** 的一半。**⑦ `.env.docker` 补齐设备上报三项前置**（`22116bbc`）：`DEVICE_REPORT_KEY=` 是**空串**，而 `verify_device_key` 的 `not expected` **恒真 → 一律 401**，**上报端点实际不可用**；另缺 `ALLOW_DEVICE_REPORT`（靠 `DEBUG` 隐式兜底）与 `OFFLINE_SCAN_ENABLED=false`（默认 `True`，60 秒扫描会刷 `offline` 并造 fault）——三项均为 `testing-guidelines.md:59` 硬要求，**此前全缺**。**⑧ 事件报告导出必然 500**（`89d63a04`）：`generate_report_html(event: Dict, alarm: Dict, timelines: list)` 形参声明是 dict，`event`/`alarm` 走了 `__dict__` 转换，**`timelines` 漏了** → 第 110 行 `.get()` 抛 `AttributeError`；因真实事件恒有 `alarm`+`confirm` 两条初始节点，**每个真实事件的报告都导不出**。**验证**：后端全量 **9 failed / 470 passed / 60 skipped / 3 xfailed**（542 条，起点 27/427/515 → 净 **−18 红/+43 绿**）、前端 **53 files / 397 passed** + build 退出码 0、**变异验证 8 次**（含一次必须做的：`Event.spec.js` 的 T6-9/T6-10 是「断言按钮不存在」的**否定式断言，实现前就是绿的**，不打变异分不清它守的是修复还是空气）、容器实测真火警全链路（`alarm_id=6` → 卡片 0→1 → 确认 → 自动建 `EV-20260919-003` + 两条时间轴同时间戳 → 四帧 WS 顺序正确 → 演练告警确认被 400 拦下）与报告导出 HTTP 200/2857 字节。⚠️ **本会话全程有 peer 会话在同一仓库并发提交**（8 个后端文件 + `Center.vue` + 5 个新测试文件，它修掉了 P1-013 等），**多数增量属它、不能记在本会话**；所有归档数字均已标注。⚠️ **本会话犯过一次方法错误并已更正**：中途读到 `confirm_alarm` 里的 `create_emergency_event` 调用并据此下结论，但当时该文件 `git status` 是 `M`——**那其实是 peer 的进行中改动、不是原代码**；结论恰好正确但**过程不可靠**。教训：**读代码下结论前先确认该文件是「已提交」还是「工作区未提交」**。⚠️ **发现「既存红淹没真实缺陷」的又一实证**：`test_emergency_report.py` 那 5 条**一直是红的**，但红在 **P1-013**（建数据那步就崩），**根本执行不到被测代码**；P1-013 一修好缺陷当场暴露——**它们不是没被发现，是没被「执行」**。**新增 DEC-024~DEC-030**。会话文件见 `.claude/sessions/2026-09-19-1259.md`。
...
> **2026-09-19 13:53 归档**：本次**未新增待办条目**，改动全部是**事实订正**（P1-010 / P1-012 / P2-021 三条备注里的计数与根因分类），另新增 **DEC-031**。**P1-013 已按维护规则移出池**（`bigint_pk()` 落地后全量套件内 `NOT NULL constraint failed: *.id` 已全部消失，`test_notifications.py` 由 13 条仅 1 条能过变为 9 passed / 4 failed，本条已过 1 个归档周期；移出记录见 `.claude/sessions/2026-09-19-1353.md`）。⚠️ **唯一待决策项**：P1-010 的优先级**列**仍为 `P1`——本会话已把「建议 P1 → P2」的理由写进备注（该服务函数无生产调用方、1 行 import 即可修、与 P2-021 的 A 类 3 条重叠），但**改列属排期判断而非事实订正**，未擅自改。
...
| P1-010 | `emergency_service.py` 少 import `update` 导致 NameError | P1 | [ ] | 2026-09-13 | **2026-09-19 更正：此前记的「影响面 = 通知已读链路，`POST /notifications/read-all` 必然 500」是错的。** 复核 `notifications.py:73-90`：该端点**第 80 行有局部 `from sqlalchemy import update`** → **端点正常，没有 NameError**。真正报错的是 `emergency_service.mark_notifications_as_read`（`:449-473`，`:467` 用 `update()` 而文件顶部 `:14` 只 import 了 `select, func`）——它被 `notifications.py:19` **import 了但没有任何端点调用**，是**死代码**。**错因**：从失败用例倒推，只看了 `notifications.py:19` 的 import 行就断定端点在调它，**没读端点函数体**。当前实际影响：**3 条测试红**（`test_notifications.py` 直接调该函数的三条：`test_mark_single_as_read` / `test_mark_nonexistent_notifications` / `test_concurrent_mark_read`，均报 `emergency_service.py:467: NameError: name 'update' is not defined`；该文件第 4 条红 `test_cannot_mark_other_users_notifications` 是**另一回事**——测试文件自己漏 import），**无线上影响**。**2026-09-19 全量测试会话独立复核，补两条证据**：（a）全仓库检索 `mark_notifications_as_read` 仅 3 类命中——定义处 `:449`、`notifications.py:19` 的**未使用 import**、`tests/test_notifications.py` 的 5 处调用，**没有任何端点调用它**（已确认路由 `read-all` 自带内联实现）；（b）**容器实测**（`docker-compose.yml` 把 `./backend` 挂载为 `/app`，已核对容器内 `notifications.py:80` 确有该内联 import、`emergency_service.py:14` 确无 `update`）→ `POST /api/v1/notifications/read-all` 返回 **HTTP 200** `{"code":200,"message":"已全部标记为已读"}`，`unread-count` 前后均为 0（空操作，未改动数据）。**修法：推荐删掉该死代码**（连同 `notifications.py:19` 的无用 import）；若决定保留并接线，则补 `update` 到顶部 import + 加回归用例。⚠️ 因当前**不可达**，建议优先级 **P1 → P2**。行号注：原记的 `:392` 已过时（文件增长，现为 `:467`）。 |
...
| P1-014 | 后端应急事件 `resolve` / `close` **无状态机校验** | P1 | [ ] | 2026-09-19 | 2026-09-19 实测：`resolve_emergency_event` / `close_emergency_event`（`emergency_service.py:320/349`）**直接赋值 `event.status`，不看当前状态**。后果：① 已 `closed` 的事件可被「处置完成」**覆盖回 `resolved`**；② `resolved` 的事件可再次处置，而 **`resolve` 写 `complete` 时间轴节点没有去重**（`close` 有）。本轮只在前端按钮的**显示条件**上规避（`status==='processing'` / `status!=='closed'`），**直接打 API 仍可绕过**。修法：两个 service 函数加状态机判断 + 用例。⚠️ 同期前端「执行」入口已补齐（`ec901531`），修后端时记得复验按钮显示条件是否随之调整 |
| P2-019 | `linkage:log`（联动日志菜单）是否对值班员开放 | P2 | [ ] | 2026-09-19 | 2026-09-19 补值班员菜单码时**刻意只补了 `linkage:plan`**，未补二级菜单 `linkage:log`——是否让值班员看联动日志是**产品口径**，不该顺带决定。现状：值班员能进「联动预案」但看不到「联动日志」子菜单。定口径时一并看 **P2-018**（该页无数据范围过滤，若对值班员开放，则「能看到全部组织的联动日志」的问题会同时暴露给更低权限角色）|
| P2-020 | `generate_report_html` 的 `event`/`alarm` 用 `dict(obj.__dict__)` 转换，脆弱 | P2 | [ ] | 2026-09-19 | 2026-09-19 修报告导出时**刻意只动了 `timelines`**（那是崩溃根因）。另两个实参是 `event.model_dump() if hasattr(event,'model_dump') else dict(event.__dict__)`——ORM 对象**没有** `model_dump`，所以永远走 `__dict__`，会带上 `_sa_instance_state` 且**只含已加载列**。当前可用无缺陷，但换个模型或加一个未加载列就会踩（异步下是 `MissingGreenlet`）。仓库已有 canonical 序列化器（`event_payload` / `timeline_payload` / `alarm_payload`），收敛过去即可 |
| P2-021 | 剩余 9 条既存红（**6 个根因，其中 1 个根因 / 3 条指向生产代码**） | P2 | [ ] | 2026-09-19 | 当前全量 9 failed 的构成见 **P1-012** 备注。**逐条实测分类（2026-09-19 更正——原记「全在测试侧 / 三类根因」不准确）**：**A. 指向生产代码——1 个根因 / 3 条**：`test_notifications.py` 的 `test_mark_single_as_read` / `test_mark_nonexistent_notifications` / `test_concurrent_mark_read` 均报 `emergency_service.py:467: NameError: name 'update' is not defined`（见 **P1-010**；该服务函数无生产调用方，属**潜伏缺陷而非线上故障**）。**B. 测试自身缺陷——5 个根因 / 6 条**：① `test_emergency_fixtures.py::test_cascade_delete_timeline` —— `EmergencyTimeline(user_id=...)` 应为 `operator_id`（P1-011 同类漂移，这次漂移的是**测试**）；② `test_emergency_fixtures.py::test_add_timeline_node` —— **实参顺序传反**（`add_timeline_node(session, event_id, node_type, operator_id)`，测试传了 `(1, "field_confirmed")`，故 `node.node_type` 读到 `'1'`）——**此前记的「一条断言写错」不准确，断言本身是对的**；③④ `test_emergency_fixtures.py` 两条 `SQLite DateTime type only accepts Python datetime`（SQLite 方言不接受字符串日期，生产 PG 不受影响）；⑤ `test_emergency_event.py::test_create_event_on_real_alarm` —— 测试第 45 行直接访问惰性关系 `event.timelines` 触发 `MissingGreenlet`（应改 `await db_session.refresh(event, ["timelines"])` 或 `selectinload`）；⑥ `test_notifications.py::test_cannot_mark_other_users_notifications` —— 测试文件缺 import（同文件 117/184/282 行都写了局部 import，唯 169 行这处漏了）。这批此前同样被 P1-013 挡着跑不到。**修法分两类**：A 类动生产代码（1 行 import，或删死代码），B 类纯改测试、不动生产代码。 |
| P1-015 | 超时升级链路（FR：待确认报警超 5 分钟升级通知主管）**决定暂不做** | P1 | [-] | 2026-09-19 | **2026-09-19 用户明确决定暂缓，不是遗漏。** 链路当前**完全不通**，且有两个**互相独立**的成因：① `emergency_service.py:177` 访问 `alarm.emergency_event` 触发懒加载，async 下 `MissingGreenlet` 崩溃；② 扫描只取 `Alarm.status == "pending"` 的报警，而应急事件只在**确认时**（pending → confirmed）创建 —— **二者互斥，闸门恒空**。成因② 由本会话独立查出、peer 会话亦独立得出同一结论。两条已由 peer 以 **`@pytest.mark.xfail(strict=True)`** 入册于 `tests/test_escalation_scan.py`——`strict=True` 意味着**哪天转绿会反过来报错**，既是记录也是探针。**重估条件**：出现真实依赖「超时升级」的场景时再评估（当前无任何角色依赖它，主管看不到升级通知也不会有人察觉）|
